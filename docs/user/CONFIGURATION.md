...
This document describes all configuration options and settings available in the PetSafe Extended custom integration.
...
### `petsafe_extended.example_service`
...
```yaml
service: petsafe_extended.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: petsafe_extended.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/ThomasHFWright/homeassistant-petsafe/issues) - Report problems
