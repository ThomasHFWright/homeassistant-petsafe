...
This document describes the technical architecture of the PetSafe Extended custom component for Home Assistant.

## Directory Structure

```text
custom_components/petsafe_extended/
...
│   ├── __init__.py          # Exports PetSafeExtendedDataUpdateCoordinator
...
│   ├── __init__.py          # Exports PetSafeExtendedEntity
...
- `base.py` - Main coordinator class (`PetSafeExtendedDataUpdateCoordinator`)
...
**Key class:** `PetSafeExtendedDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `PetSafeExtendedApiClient`
...
- `PetSafeExtendedConfigFlowHandler` (main flow)
- `PetSafeExtendedOptionsFlow` (options)
...
**Key class:** `PetSafeExtendedEntity` (in `entity/base.py`)
...
2. `PetSafeExtendedEntity` for common functionality
...
1. Create directory: `custom_components/petsafe_extended/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `PetSafeExtendedEntity`
