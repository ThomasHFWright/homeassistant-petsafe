...
- **Repository:** ThomasHFWright/homeassistant-petsafe

**Key directories:**

- `custom_components/petsafe_extended/` - Main integration code
...
- Integration logs: `custom_components.petsafe_extended: debug` in `config/configuration.yaml`
...
- Use the domain `petsafe_extended` for all DOMAIN references
- Prefix all integration-specific classes with `PetSafeExtended`
- Use "PetSafe Extended" as the display title
...
- Inherit from platform base + `PetSafeExtendedEntity`
...
- `tests/` mirrors `custom_components/petsafe_extended/` structure
